...
LevelWindowBase.prototype.Update = function (elapsed) {
    ...
    if (this.state == LEVELINF_STATE_GAMELOADING) {
        ...
        var total = g_resourceLoadCount + g_audioResourceList.length;
        var loaded = g_imageResourceList.length + g_errorImageList.length + audio_loaded;
        this.loadingPct = (loaded / total);
    ...
    /*if (this.levelToLoad == g_gameData.top_level) {*/
        switch (this.levelToLoad) {
            ...
            case 21:
                ...
                break;
            case 43:                
                g_Engine.SetState(SLIDE_STORY_STATE_2);
                break;
            case 65:
                g_Engine.SetState(SLIDE_STORY_STATE_3);
                break;
            case 89:
                g_Engine.SetState(SLIDE_STORY_STATE_4);
                break;
            case 39:
                g_Engine.SetState(SLIDE_STORY_STATE_5);                
                break;
            default:
...
LevelWindowBase.prototype.Draw = function (gfx) {
    ...
    if (this.state == LEVELINF_STATE_GAMELOADING) {
        ...
        var barwidth = (nwidth)/2;
        var barheight = 20;
        gfx.DrawRect(
            (DEFAULT_WINDOW_WIDTH / 2) - (barwidth / 2),
            500,
            barwidth, barheight,
            "rgb(255,255,255)", 1);
        
        var greenbar_width = barwidth * this.loadingPct;
        gfx.FillRect(
            (DEFAULT_WINDOW_WIDTH / 2) - (barwidth / 2),
            500, greenbar_width, barheight,
            "rgb(0,255,0)", 0.5);

        var ctx = gfx._canvasBufferContext;
        var style = "12pt ERASDEMI";
        ctx.font = style;
        var text = Math.floor(this.loadingPct*100) + "%";
...
LevelWindowBase.prototype.LoadGame = function (next) {
    ...
    var gameAudioList = [g_gameAudioCommonList_1, g_world1Audio_List,
        g_world2Audio_List, g_world3Audio_List, g_world4Audio_List];

    // story slides
    var storyImageList = [g_slides1_images, g_slides2_images,
        g_slides3_images, g_slides4_images, g_slides5_images,
        g_slide6_images, g_finalStory];

    var storyAudioList = [g_storyAudio_1, g_storyAudio_2];
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    if (true/*this.levelToLoad == g_gameData.top_level*/) {
        var index = 0;
        switch (this.levelToLoad) {
            case 0:
                index = 0;
        ...
        if (index != -1) {
            if (index < storyImageList.length && !g_storySlideImage[index]) {
                ...
                if (index < 2) {
                    ...
                    new AudioResource().Load(g_storyAudio_2[0]);
                }
                g_audioLoadCount++;
            }           
        }
    ...
    if (true/*this.state == LEVELINF_STATE_IDLE*/) {
        ...
        for (var i = 0; i < description.length; i++) {
            ...
            gfx.DrawText(description[i],
                189, this._Y + 284 + (i * 30), "rgb(253,255,110)", "18pt ERASDEMI");
