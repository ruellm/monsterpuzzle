...
LoadState.prototype.Update = function (elapsed) {
    ...
    if ( g_imageResourceList.length >= g_imageFileList.length ) {
        ...
        if (/*this.audio_loaded >= g_audioFileList.length*/1) {		
			DEBUG_LOG("ALL Loading done...");
			DEBUG_LOG("Loading flag " + g_gameData.isDataLoadDone);
			DEBUG_LOG("Error flag " + this.error );			
			DEBUG_LOG("Audio loading is by passed!!");
			
			if( g_gameData.isDataLoadDone ){
				DEBUG_LOG("Main user id " + g_mainUser.id);
				if( g_mainUser.id != -1 || g_DEBUG_MODE) {
...
LoadState.prototype.Draw = function (gfx) {
	...
	if( this.error ){
		var ctx = gfx._canvasBufferContext;
		var style = "Bold 30pt ERASDEMI";
		ctx.font = style;
		
		var text = "Error encountered during loading,";
		var textWidth = ctx.measureText(text);
		var x = (DEFAULT_WINDOW_WIDTH/2) - (textWidth.width / 2);
		var y = (DEFAULT_WINDOW_HEIGHT/2);
		gfx.DrawText(text,
			x, y, "rgb(255,255,255)", style);
			
		var text = "Please Reload the game...";
		var textWidth = ctx.measureText(text);
		var x = (DEFAULT_WINDOW_WIDTH/2) - (textWidth.width / 2);
		gfx.DrawText(text,
			x, y+35, "rgb(255,255,255)", style);
	}
}
