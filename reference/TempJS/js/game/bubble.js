...
Bubble.prototype.Update = function (elapsed) {
    ...
    if (this.state == BUBBLE_STATE_FIRED) {
		DEBUG_LOG("State Fired!");
		if(!this.doneFying && 
			this.currentTargetIndex < this.targetArray.length){
			DEBUG_LOG("flying Confirmed");
			var tempX = this._X + (this.deltaX * BUBBLE_SPEED) * elapsed;
			var tempY = this._Y + (this.deltaY * BUBBLE_SPEED) * elapsed;
			var error = 1;
			var ctr = 0;
			...
			if (this.deltaX > 0) {
				...
				if (tempX + (RADIUS * 2) >= this.fieldXLimitCopy) {
					...
					DEBUG_LOG("Target X " + targetX);
					DEBUG_LOG("current X " + this._X);
					DEBUG_LOG("Temp X " + tempX);
				} else {
					this._X = targetX;
					ctr++;
				}
